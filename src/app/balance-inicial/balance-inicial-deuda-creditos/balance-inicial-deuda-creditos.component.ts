...
import { BalanceInicialDetalleDeudaCreditosComponent } from '../balance-inicial-deuda-creditos/balance-inicial-detalle-deuda-creditos/balance-inicial-detalle-deuda-creditos.component';
...
export class BalanceInicialDeudaCreditosComponent extends ConsultasBaseComponent implements OnInit {
  ...
  constructor(public peticiones: PeticionesWebComponent,
    ...
    private datePipe: DatePipe) {
      ...
      this.Nombrecatalogo = 'Deuda Creditos' 
    }
 ...
 */
   consulta() {
    this.mostrarCargado();
    this.peticiones.peticionPost({}, 'consultaDCMovIni').then((resultado: any) => {
      (resultado);
  ...
  */
  configuraDataGrid(): void {
    let configGrid = {
      columns: 7,
      header: ['#', 'Fecha','Folio','Forma de Recibirlo', 'Crédito', 'Importe', 'Plazo'],
      field: ['NUM', 'FECHA', 'FOLIO', 'RECIBIDO_EN', 'NOM_CREDITO', 'IMPORTE', 'PLAZO'],
 ...
 */
  ...
  */
  ventanaDetalle(Modo) {
    ...
    const dialogRef = this.dialog.open(BalanceInicialDetalleDeudaCreditosComponent
      , {
      disableClose: true,
      width: width,
      height: height,
