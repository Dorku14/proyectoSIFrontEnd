...
@Component({
  ...
  templateUrl: './balance-inicial-credito-proveedores.component.html',
  styleUrls: ['./balance-inicial-credito-proveedores.component.scss']
})
export class BalanceInicialCreditoProveedoresComponent implements OnInit {
  ...
  datos = { 'NUM': 0, 'FECHA': '', 'PROVEEDOR': '', 'FACTURA': '', 'IMPORTE': 0, 'IVA': 0 };
  ...
  agregar() {
    ...
    if (!this.validaCamposVacios()) {
      ...
      this.datos = { 'NUM': 0, 'FECHA': '', 'PROVEEDOR': '', 'FACTURA': '', 'IMPORTE': 0, 'IVA': 0 }
    }
  }
  ...
  validaCamposVacios(): boolean {
    ...
    let buscar = this.dataSource.find(item => item.FACTURA === '');
    if (buscar) {
      campoVacio = true;
      let indice = this.dataSource.findIndex(item => item.FACTURA === '');
