 ...
 */
  consulta() {
    ...
    this.peticiones.peticionPost({}, 'consultaProductosF').then((resultado: any) => {
      ...
      for (let i of this.dataSource) {
        num += 1;
        i.NUM = num;
        i.COSTO_PRODUCCION = this.funcGenerales.dameFormatoMoneda(i.COSTO_PRODUCCION,2);
  ...
  */
  configuraDataGrid(): void {
    let configGrid = {
      columns: 8,
      header: ['#', 'Código','Producto','Cantidad (Balance Inicial)', 'Costo de producción (Sin IVA)', 'Precio de venta(Sin IVA)','Costo actualizado','Precio de venta actualizado'],
      field: ['NUM', 'CODIGO', 'PRODUCTO','CANTIDAD', 'COSTO_PRODUCCION','PRECIO_VENTA', 'COSTO_ACTUALIZADO','PRECIO_VENT_ACT'],
